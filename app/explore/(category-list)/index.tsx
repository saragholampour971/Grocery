...
const colors = [
  "#53B175", '#F8A44C', '#F7A593', '#D3B0E0', '#FDE598', '#B7DFF5',
]
...
export default async function CategoryList() {
  ...
  return (
    <div className={'flex flex-wrap gap-[15px] justify-between px-app-padding mb-1'}>
      {Array.isArray(data) ?
        data?.map((item, index) =>
          <Link
            ...
            <Card
              ...
              className={'text-center flex flex-col justify-between w-full h-full'}
            >

              <div className={'relative w-full h-[calc(100vw/4)] '}>
                <Image
                  fill
                  src={item.imageUrl} alt={item.title} className={'object-contain p-1'}
                />
              </div>
